...
if (theme_get_setting('nurani_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('nurani_zen_tabs')) {
...
function nurani_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('nurani_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = nurani_id_safe('page-'. $path);
    $body_classes[] = nurani_id_safe('section-'. $section);
...
function nurani_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = nurani_id_safe('node-type-' . $vars['type']);
  $vars['classes'] = implode(' ', $classes); // Concatenate with spaces
}

function nurani_preprocess_comment_wrapper(&$vars) {
  $classes = array();
...
function nurani_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = nurani_id_safe('block-' . $vars['block']->module);
    $classes[] = nurani_id_safe('block-' . $vars['block']->region);
    $classes[] = nurani_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('nurani_block_editing') && user_access('administer blocks')) {
...
function nurani_preprocess_comment(&$vars, $hook) {
...
function nurani_menu_item_link($link) {
...
function nurani_menu_local_tasks() {
  $output = '';
...
function nurani_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = nurani_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function nurani_id_safe($string) {
...
*/
function nurani_breadcrumb($breadcrumb) {
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('nurani_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('nurani_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('nurani_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('nurani_breadcrumb_title')) {
        if ($title = drupal_get_title()) {
          $trailing_separator = $breadcrumb_separator;
        }
      }
      elseif (theme_get_setting('nurani_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
